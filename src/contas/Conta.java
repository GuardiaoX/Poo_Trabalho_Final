...
public abstract class Conta extends Cliente {
	...
	public Boolean verificaSaldo(BigDecimal valor, int tipoOperacao, int tributacaoTipo) {
		if (tributacaoTipo == TributacaoTipo.TRIB_PAD.getId()) {
			if (tipoOperacao == 1) {
				...
				Boolean compareTo = compareToVeriSal(valor);
				return compareTo;
	...
	public Boolean verificaEntrada(BigDecimal valor) {
		if (valor.compareTo(paramZero) == 1 | valor.compareTo(paramZero) == 0) {
	...
	public Boolean tributacaoAcumuladoCalculo() {
		if (TributacaoTipo.TRIB_PAD.getId() == 1) {
			BigDecimal paramTribDep = new BigDecimal("0.00");
			BigDecimal paramTribSq = new BigDecimal("0.00");
			BigDecimal paramTribTransf = new BigDecimal("0.00");
			BigDecimal paramTribGeral = new BigDecimal("0.00");
